...
public class SecureSyncTime implements ILockDataOperator {

    private final String TAG = "SecureSyncTime";
     ...
     */
    public SecureSyncTime(Activity activity, Device mDevice) {
        ...
        XmBluetoothManager.getInstance().securityChipEncrypt(mDevice.getMac(), cmd, new Response.BleReadResponse() {
            ...
            public void onResponse(int i, byte[] bytes) {//bytes加密后的数据
                if (i == XmBluetoothManager.Code.REQUEST_SUCCESS) {
                    ...
                    LockCommSyncTime lockCommSyncTime = new LockCommSyncTime(bytes);//封装进锁通讯
                    byte[] commBytes = lockCommSyncTime.getBytes();//锁通讯命令
    ...
    private void processCharacterChanged(Intent intent) {
        ...
        if (MyEntity.RX_SERVICE_UUID.equals(service) && MyEntity.TX_CHAR_UUID.equals(character)) {
            ...
            try{
                final LockCommSyncTimeResponse lockCommSyncTimeResponse = (LockCommSyncTimeResponse) LockCommBase.parse(inputBuffer);
                if(lockCommSyncTimeResponse == null) return;
                ...
                Log.d(TAG, "错误码:"+lockCommSyncTimeResponse.getResultCode());
