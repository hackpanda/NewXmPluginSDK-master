...
public class SecurePwdUpdate implements ILockDataOperator {

    private final String TAG = "SecurePwdUpdate";
     ...
     */
    public SecurePwdUpdate(Activity activity, Device mDevice) {
        this.activity = activity;
        this.mDevice = mDevice;
    }
    @Override
    public void sendLockMsg(final byte[] cmd, LockOperateCallback mlockOperateCallback) {
        inputBuffer.clear();//清空缓存
        this.lockOperateCallback = mlockOperateCallback;
    ...
    private void processCharacterChanged(Intent intent) {
        ...
        if (MyEntity.RX_SERVICE_UUID.equals(service) && MyEntity.TX_CHAR_UUID.equals(character)) {
            ...
            }catch (LockFormatException lfe){
                Log.d(TAG, "LockFormatException");
                lfe.printStackTrace();
            }catch (ClassCastException cce){
                cce.printStackTrace();
                lockOperateCallback.lockOperateFail("密码修改失败");
            }
        }
    }
