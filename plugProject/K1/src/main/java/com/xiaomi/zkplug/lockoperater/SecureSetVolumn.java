...
import cn.zelkova.lockprotocol.LockCommSetVolumn;
import cn.zelkova.lockprotocol.LockCommSetVolumnResponse;
...
public class SecureSetVolumn implements ILockDataOperator {

    private final String TAG = "SecureSetVolumn";
     ...
     */
    public SecureSetVolumn(Activity activity, Device mDevice) {
        ...
        XmBluetoothManager.getInstance().securityChipEncrypt(mDevice.getMac(), cmd, new Response.BleReadResponse() {
            ...
            public void onResponse(int i, byte[] bytes) {//bytes加密后的数据
                if (i == XmBluetoothManager.Code.REQUEST_SUCCESS) {
                    ...
                    LockCommSetVolumn lockCommSetVolumn = new LockCommSetVolumn(bytes);//封装进锁通讯
                    byte[] commBytes = lockCommSetVolumn.getBytes();//锁通讯命令
                    ...
                    //Toast.makeText(activity, activity.getString(R.string.gloable_encrypt_fail)+":"+i, Toast.LENGTH_SHORT).show();
    ...
    private void processCharacterChanged(Intent intent) {
        ...
        if (MyEntity.RX_SERVICE_UUID.equals(service) && MyEntity.TX_CHAR_UUID.equals(character)) {
            ...
            try{
                final LockCommSetVolumnResponse lockCommSetVolumnResponse = (LockCommSetVolumnResponse) LockCommBase.parse(inputBuffer);
                if(lockCommSetVolumnResponse == null) return;
                ...
                Log.d(TAG, "错误码:"+lockCommSetVolumnResponse.getResultCode());
                if(lockCommSetVolumnResponse.getResultCode() == 0){
                    lockOperateCallback.lockOperateSucc(activity.getString(R.string.device_mute_set_succ));
                }else{
