...
import cn.zelkova.lockprotocol.LockCommGetLog;
...
public class SecureLogRead implements ILogReadOperator {

    private final String TAG = "SecureLogRead";
    ...
    LogReadCallback logReadCallback;//操作锁的回调
    /**
     * @param activity
     * @param mDevice
     * 描述：赋值, 用于设备和Toast提示的activity
     */
    public SecureLogRead(Activity activity, Device mDevice, final long beginIdx) {
        this.activity = activity;
        this.mDevice = mDevice;
        this.beginIdx = beginIdx;
    }
    @Override
    public void sendLockMsg(final byte[] cmd,  final LogReadCallback logReadCallback) {
        inputBuffer.clear();//清空缓存
        this.logReadCallback = logReadCallback;
        XmBluetoothManager.getInstance().securityChipEncrypt(mDevice.getMac(), cmd, new Response.BleReadResponse() {
            ...
            public void onResponse(int i, byte[] bytes) {//bytes加密后的数据
                if (i == XmBluetoothManager.Code.REQUEST_SUCCESS) {
                    ...
                    LockCommGetLog lockCommGetLog = new LockCommGetLog(bytes);
                    byte[] commBytes = lockCommGetLog.getBytes();//锁通讯命令
                    ...
                    do{
                        ...
                        if (surplusData.length <= 20) {
                            ...
                            Log.d(TAG, "currentData-last:"+cn.zelkova.lockprotocol.BitConverter.toHexString(currentData));
    ...
    private void processCharacterChanged(Intent intent) {
        ...
        if (MyEntity.RX_SERVICE_UUID.equals(service) && MyEntity.TX_CHAR_UUID.equals(character)) {
            ...
            try{
                LockCommGetLogResponse lockCommGetLogResponse = (LockCommGetLogResponse) LockCommBase.parse(inputBuffer);
                if(lockCommGetLogResponse == null) return;
                Log.d(TAG, "锁返回数据:"+BitConverter.toHexString(buffer)+"错误码："+lockCommGetLogResponse.getResultCode());
                Collections.addAll(inputBuffer, BitConverter.toPackaged(buffer));
                Log.d(TAG, "进锁成功:inputBuffer.size()"+inputBuffer.size());
